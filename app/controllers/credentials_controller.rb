class CredentialsController < ApplicationController
  ...
  def index
    @credentials = Credential.find(:all, :order => 'depot_id')
   
    if (params[:depot])
        if (params[:depot][:id] != "")
          #@credentials = Credential.find_by_project_id(params[:depot][:name].downcase)
          logger.debug "************** PARAMS: #{params[:depot][:id]}"
          @credentials = Credential.all(:conditions => {:depot_id => params[:depot][:id]})
        end
    else
      @credentials = Credential.find(:all, :order => 'depot_id')
    end
